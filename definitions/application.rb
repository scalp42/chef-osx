define :application do
  ...
  already_installed = "#{not_if_conditions} || test -d '#{node[:applications][:dir]}/#{app_name}' || test -d '#{node[:library]}/PreferencePanes/#{app_name}'"
  ...
  when :tbz
    ...
    script "install #{app_name}" do
      ...
      code <<-EOS
        ...
        if [ -e *.prefPane ]; then
          cp -r "#{app_name}" #{node[:library]}/PreferencePanes/
        fi
      EOS
    end
  ...
  when :dmg
    ...
    unless params[:noop]
      script "install #{app_name}" do
        ...
        code <<-EOS
          if [ -e *.mpkg ]; then
            for mpkg in $(ls -c1 | grep .mpkg); do
              echo "Installing ${mpkg}..."
              installer -verbose -pkg $mpkg -target /
            done
          else
            if [ -e *.app ]; then
              cp -rv "#{app_name}" #{node[:applications][:dir]}/
            else
              if [ -e *.prefPane ]; then
                cp -rv "#{app_name}" #{node[:library]}/PreferencePanes/
              else
    ...
    execute "verify #{app_name} is installed" do
      command already_installed
    end
  else
